...
void Waifu::Update(float dt)
{
    ...
    if(Input::GetInstance()->GetKeyDown(SDL_SCANCODE_W)&&!m_IsWalling){
        m_IsCrouching=true;
        if(m_Flip==SDL_FLIP_NONE) m_Rigidbody->ApplyForceX(2.5*FORWARD);
        else  m_Rigidbody->ApplyForceX(2.5*BACKWARD);
    }

    if(Input::GetInstance()->GetKeyDown(SDL_SCANCODE_S)&&!m_IsWalling){
        m_IsDashing=true;
        if(m_Flip==SDL_FLIP_NONE) m_Rigidbody->ApplyForceX(1*FORWARD);
        else  m_Rigidbody->ApplyForceX(1*BACKWARD);
    }
    if(Input::GetInstance()->GetKeyDown(SDL_SCANCODE_L)&&!m_IsWalling&&!m_IsJumping){
        m_IsSliding=true;
    ...
    if(m_Transform->X<=cam.X) m_Collider->Set(cam.X,m_Transform->Y,40,40);
    ...
    else m_Collider->Set(m_Transform->X,m_Transform->Y,40,40);
...
void Waifu::State()
{
    ...
    if(m_IsRunning) {m_Animation->SetProps("Run",1024/8,88,1,8,125);m_Collider->SetBuffer(-50,-20,0,-20);}
    if(m_IsJumping) {m_Animation->SetProps("Jump",384/3,88,1,3,150);m_Collider->SetBuffer(-50,-20,0,-20);}
    if(m_IsAttacking) {m_Animation->SetProps("Attack",1536/12,88,1,12,100);m_Collider->SetBuffer(-50,-20,0,-20);}
    if(m_IsDashAttacking) {m_Animation->SetProps("DashAttack",1380/10,88,1,10,100);m_Collider->SetBuffer(-50,-20,0,-20);}
    if(m_IsDashing) {m_Animation->SetProps("Dash",966/7,88,1,7,125);m_Collider->SetBuffer(-50,-20,0,-20);}
    if(m_IsFalling) {m_Animation->SetProps("Fall",384/3,88,1,3,150);m_Collider->SetBuffer(-50,-20,0,-20);}
    if(m_IsHurting) {m_Animation->SetProps("Hurt",512/4,88,1,4,100);m_Collider->SetBuffer(-50,-20,0,-20);}
    if(m_IsSliding) {m_Animation->SetProps("Slide",690/5,88,1,5,200);m_Collider->SetBuffer(-20,-80,-40,40);}
    if(m_IsCrouching) {m_Animation->SetProps("Crouch",828/6,88,1,6,125);m_Collider->SetBuffer(-50,-20,0,-20);}
    if(m_IsWalling) {m_Animation->SetProps("WallSlide",414/3,88,1,3,125);m_Collider->SetBuffer(-70,-20,40,-20);}
